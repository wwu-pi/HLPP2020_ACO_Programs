...
__global__ void packing_kernel(double*	d_n_objects_types, double* d_n_objects_total,
		...
		double*  d_bpp_items_copy, int* d_bins, double*  d_eta, double*  d_tau, double*  d_probs,
...
//	printf("\n n antes %i", d_n_ants[0]);
	...
	for(int i = 0 ; i < (int)2*n_objects_type ; i++){
		int index = (((int)ant_index)*(n_objects_type*2)) + i;
		d_bpp_items_copy[index] = d_bpp_items[i];
	}

	//prefix
	int bpp_items_copy_index = (int)ant_index*n_objects_type*2;
	...
	for(int i = 0 ; i < n_objects_type; i++){
		new_object_weight = d_bpp_items_copy[bpp_items_copy_index + (2*i)];
		object_quantity = d_bpp_items_copy[bpp_items_copy_index + (2*i) +1];
	...
	d_bpp_items_copy[bpp_items_copy_index + (object_index*2) + 1] -= 1.0;
...
//		}
		...
		for (int index_object_j = 0; index_object_j < n_objects_type; index_object_j++) {
			...
			double weight_object_j = d_bpp_items_copy[bpp_items_copy_index + (2*index_object_j)];
			int quantity_object_j = (int)d_bpp_items_copy[bpp_items_copy_index + (2*index_object_j)+1];
...
//			}
			...
			double weight_object_j = d_bpp_items_copy[bpp_items_copy_index + 2*selected_object];
...
//			printf("\n packing Item %i      - Weight %f ", object_j, weight_object_j);
			...
			d_bpp_items_copy[bpp_items_copy_index + (2*selected_object) + 1] -= 1.0;
			n_items_in_actual_bin++;
...
//			printf("\n\n New BIN ");
			...
			for(int k = 0 ; k < n_objects_type ; k++){
				...
				object_quantity = d_bpp_items_copy[bpp_items_copy_index + 2*k+1];
				new_object_weight = d_bpp_items_copy[bpp_items_copy_index + 2*k];
...
//	Create and Allocate device variables
    ...
    double* bpp_items = (double*)malloc(2*n_objects_types[0]*sizeof(double));

    readBPPFile(problem_id, n_objects_types, n_objects_total, bin_capacity, bpp_items);
    ...
    cudaMalloc((void**) &d_bpp_items, 2 * n_object_type *sizeof(double));
    cudaMalloc((void**) &d_bpp_items_copy, 2 * n_object_type * n_ant *sizeof(double));
...
//    checkError(0);
	...
	cudaMemcpy(d_bpp_items, bpp_items, 2*n_object_type*sizeof(double), cudaMemcpyHostToDevice);
	...
	while(iteration < n_iterations){
		...
		packing_kernel<<<n_threads, n_blocks>>>(d_n_objects_type, d_n_objects_total, d_bin_capacity, d_n_ants,
				d_phero, d_bpp_items, d_bpp_items_copy, d_bins, d_eta, d_tau, d_probabilities,d_fitness, d_rand_states_ind);
		cudaDeviceSynchronize();
		checkError(1);
		...
		//Pheromone Deposit
		update_pheromones_kernel<<<n_threads, n_blocks>>>(d_n_objects_total, d_n_objects_type, d_bin_capacity, d_phero, d_bpp_items, d_bins, d_fitness);
		cudaDeviceSynchronize();
		checkError(3);
